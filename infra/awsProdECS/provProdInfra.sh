...
export STATE_RES="$STATE_RES_NAME"
...
export AMI_ID=$(shipctl get_resource_version_name $RES_AMI)
...
set_context(){
  ...
  echo "prod_public_sn_01_id = \"$PROD_PUBLIC_SN_01_ID\"" >> terraform.tfvars
  echo "prod_public_sn_02_id = \"$PROD_PUBLIC_SN_02_ID\"" >> terraform.tfvars
...
apply_changes() {
  ...
  shipctl put_resource_state $OUT_RES_SET versionName \
    "Version from build $BUILD_NUMBER"

  shipctl put_resource_state $OUT_RES_SET PROV_STATE "Active"
  shipctl put_resource_state $OUT_RES_SET REGION $REGION
  shipctl put_resource_state $OUT_RES_SET PROD_ECS_INS_0_IP \
    $(terraform output prod_ecs_ins_0_ip)
  shipctl put_resource_state $OUT_RES_SET PROD_ECS_INS_1_IP \
    $(terraform output prod_ecs_ins_1_ip)
  shipctl put_resource_state $OUT_RES_SET PROD_ECS_INS_2_IP \
    $(terraform output prod_ecs_ins_2_ip)
  shipctl put_resource_state $OUT_RES_SET PROD_ECS_CLUSTER_ID \
